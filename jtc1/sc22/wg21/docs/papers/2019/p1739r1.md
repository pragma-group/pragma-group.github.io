...
1. In the changes for `view::take` and `view::drop`, also consider constructors of the input type that take iterator and sentinel as arguments (instead of just a constructor that takes a subrange over the two).
...
2. We have added such an (iterator, sentinel)-constructor to `ranges::empty` view so that it becomes one of the types that benefit from the changes to `view::take` and `view::drop`.

These changes were approved by LEWG as part of the discussion of R0.
...
[D1664](https://thephd.github.io/vendor/future_cxx/papers/d1664.html) defines concepts that this proposal could use to specify its behaviour more elegantly.
It generalises the notion of `reconstructible-ranges` and make more ranges "reconstructible" that can then benefit from the optimisations/fixes described here.
...
[P1252](https://wg21.link/P1252) (accepted) modified `view::reverse` to "undo" the previous reversal when passed a type that already
...
+ - T{ranges::begin(E), ranges::begin(E) + min<iter_­difference_­t<iterator_t<decltype((E))>>>(ranges::size(E), F)} if that is well-formed and T models forwarding-range.
...
These wording changes depend on [P1391](https://wg21.link/p1391) & [P1394](https://wg21.link/p1394) – however the range constructors are no longer required, because we look for (iterator, sentinel)-constructors, too.
...
+ - T{ranges::begin(E) + min<iter_­difference_­t<iterator_t<decltype((E))>>>(ranges::size(E), F), ranges::end(E)} if that is well-formed and T models forwarding-range.
...
### Changes to `empty_view`

The following changes make `ranges::empty_view` a type that gets special treatment from `view::take` and `view::drop`.
...
+
+1 Expects: first == nullptr and last == nullptr.
+
+2 Effects: none.
```
...
Many thanks to JeanHeyd Meneide for alerting me to and involving me in D1664 which would solve some of the things described here more elegantly (and more generically).
...
## References
  ...
  * [P1391](https://wg21.link/p1391) Range constructor for `std::string_view`
  * [P1394](https://wg21.link/p1394) Range constructor for `std::span`
  * [D1664](https://thephd.github.io/vendor/future_cxx/papers/d1664.html) Reconstructible Ranges
